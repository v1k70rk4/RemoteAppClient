...
public sealed class EditTokenForm : MaterialForm
{
    ...
    public EditTokenForm(BootstrapTokenInfo t)
    {
        ...
        {
            Text = L.Format(L.EditTokenForm_UsedSoFarInstallsMax, t.UseCount),
        ...
        _expiry.Items.AddRange([
            new ExpiryItem(null, false, true, L.EditTokenForm_ExpiryUnchanged),
            new ExpiryItem(null, true, false, L.EditTokenForm_NoExpiry),
            new ExpiryItem(24, false, false, L.EditTokenForm_X24HoursFromNow),
            new ExpiryItem(168, false, false, L.EditTokenForm_X7DaysFromNow),
            new ExpiryItem(720, false, false, L.EditTokenForm_X30DaysFromNow),
        ]);
        _expiry.SelectedIndex = 0;

        _maxUses.Width = 360; _maxUses.Location = new Point(20, 168);
        _maxUses.Items.Add(new UsesItem(null, true, L.EditTokenForm_MaxInstallsUnchanged));
        _maxUses.Items.Add(new UsesItem(100000, false, L.EditTokenForm_Unlimited));
        foreach (var n in new[] { 1, 5, 10, 50 })
            if (n >= t.UseCount) _maxUses.Items.Add(new UsesItem(n, false, n.ToString()));
        _maxUses.SelectedIndex = 0;

        var ok = new MaterialButton { Text = L.EditTokenForm_Save, Location = new Point(212, 220), AutoSize = false, Width = 88 };
        var cancel = new MaterialButton { Text = L.ConsentWaitForm_Cancel, Location = new Point(304, 220), AutoSize = false, Width = 86, Type = MaterialButton.MaterialButtonType.Outlined, HighEmphasis = false };
        ok.Click += (_, _) =>
        {
