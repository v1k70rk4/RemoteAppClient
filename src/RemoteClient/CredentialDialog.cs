...
public sealed class CredentialDialog : MaterialForm
{
        ...
        string? secretLabel = null, string? infoText = null)
    {
        ...
        var user = new MaterialTextBox2 { Hint = L.CredentialDialog_User, Text = username, ReadOnly = true, Dock = DockStyle.Fill, Margin = new Padding(3, 6, 3, 6) };
        var pass = new MaterialTextBox2 { Hint = secretLabel, Text = secret, ReadOnly = true, Dock = DockStyle.Fill, Margin = new Padding(3, 6, 3, 6) };
        var copy = new MaterialButton { Text = L.CredentialDialog_CopyUser + secretLabel.ToLowerInvariant() + ")", Dock = DockStyle.Fill, Margin = new Padding(3, 8, 3, 6), Type = MaterialButton.MaterialButtonType.Outlined, HighEmphasis = false };
        copy.Click += (_, _) =>
        {
            try { Clipboard.SetText(L.Format(L.CredentialDialog_User_2, username, secretLabel, secret)); copy.Text = L.CredentialDialog_CopiedToClipboard; }
            catch { copy.Text = L.CredentialDialog_ClipboardIsBusyTryAgain; }
        };
        var info = new MaterialLabel
        {
            Text = infoText ?? L.CredentialDialog_OnFirstSignInThe,
