...
public sealed class NewUserForm : MaterialForm
{
    ...
    private readonly MaterialTextBox2 _name = new() { Hint = L.NewUserForm_DisplayNameForExampleJohn };
    private readonly MaterialTextBox2 _email = new() { Hint = L.NewUserForm_EmailRequired };
    private readonly MaterialComboBox _role = new() { Hint = L.NewUserForm_Role };
    private readonly MaterialSwitch _emailCode = new() { Text = L.NewUserForm_SendResetCodeByEmail, AutoSize = true, Checked = true };
    ...
    public NewUserForm()
    {
        ...
        var ok = new MaterialButton { Text = L.NewUserForm_Create, DialogResult = DialogResult.OK, AutoSize = true };
        var cancel = new MaterialButton { Text = L.ConsentWaitForm_Cancel, DialogResult = DialogResult.Cancel, AutoSize = true, Type = MaterialButton.MaterialButtonType.Outlined, HighEmphasis = false };
        ok.Click += (_, _) =>
        {
            if (Username.Length == 0) { DialogResult = DialogResult.None; MessageBox.Show(L.NewUserForm_EnterAUsername); return; }
            if (string.IsNullOrWhiteSpace(Email) || !Email!.Contains('@')) { DialogResult = DialogResult.None; MessageBox.Show(L.NewUserForm_EnterAValidEmailAddress); }
        };
